...
def training_loop(dataloader_X, dataloader_Y, test_dataloader_X, test_dataloader_Y, opts):
    ...
    for iteration in range(1, opts.train_iters+1):
        ...
        D_X_loss = torch.mean(torch.pow((D_X(images_X)-1),2))
        D_Y_loss = torch.mean((D_Y(images_Y)-1)**2)
        #print(D_X_loss.data[0])
        #print(D_Y_loss.data[0])

        d_real_loss = D_X_loss + D_Y_loss

        #print(d_real_loss.data[0])
        ...
        if opts.use_cycle_consistency_loss:
            ...
            cycle_consistency_loss = torch.mean(torch.norm(images_Y-reconstructed_Y)**2)
        ...
        if opts.use_cycle_consistency_loss:
            ...
            # 3. Compute the cycle consistency loss (the reconstruction loss)
            cycle_consistency_loss = torch.mean(torch.norm(images_X-reconstructed_X)**2)
            g_loss += cycle_consistency_loss
